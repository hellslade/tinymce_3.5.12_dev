...
function insertTable() {
	...
	border = formObj.elements['border'].value !== "" ? formObj.elements['border'].value :
		"";
	cellpadding = formObj.elements['cellpadding'].value !== "" ? formObj.elements[
		'cellpadding'].value : "";
	cellspacing = formObj.elements['cellspacing'].value !== "" ? formObj.elements[
		'cellspacing'].value : "";
	...
	if (colLimit && cols > colLimit) {
		tinyMCEPopup.alert(inst.getLang('table_dlg.col_limit').replace(/\{\$cols\}/g,
			colLimit));
		return false;
	} else if (rowLimit && rows > rowLimit) {
		tinyMCEPopup.alert(inst.getLang('table_dlg.row_limit').replace(/\{\$rows\}/g,
			rowLimit));
		return false;
	} else if (cellLimit && cols * rows > cellLimit) {
		tinyMCEPopup.alert(inst.getLang('table_dlg.cell_limit').replace(
			/\{\$cells\}/g, cellLimit));
		return false;
	}
	...
	if (action == "update") {
		...
		} else if (inst.settings.table_style_by_css || (border !== '' && isCssSize(
				border))) {
		...
		if (formObj.width.value != orgTableWidth || formObj.height.value !=
			orgTableHeight)
	...
	tdWidth_percent = Math.floor(100 / cols) + "%";
	if (inst.settings.table_style_by_css) {
		...
		newCellStyles["width"] = tdWidth_percent;
		...
		cellStyles = " style='width:" + tdWidth_percent + ";'";
	}
...
function init() {
	...
	var cols = 2,
		rows = 2,
		border = tinyMCEPopup.getParam('table_default_border', ''),
	...
	tinymce.each("summary id rules dir style frame".split(" "), function(name) {
		var tr = tinyMCEPopup.dom.getParent(name, "tr") || tinyMCEPopup.dom.getParent(
			"t" + name, "tr");
	...
	if (elm && action != "insert") {
		...
		cellpadding = dom.getAttrib(elm, 'cellpadding', '') || getTDTHOverallStyle(
			elm, 'padding');
		cellspacing = dom.getAttrib(elm, 'cellspacing', '') || trimSize(getStyle(elm,
			'border-spacing'));
		...
		background = getStyle(elm, 'background', 'backgroundImage').replace(new RegExp(
			"url\\(['\"]?([^'\"]*)['\"]?\\)", 'gi'), "$1");
...
function changedBorder() {
	...
	if (formObj.border.value !== "" && (tinyMCEPopup.editor.settings.table_style_by_css ||
			isCssSize(formObj.border.value)))
...
function changedStyle() {
	...
	if (st['background-image'])
		formObj.backgroundimage.value = st['background-image'].replace(new RegExp(
			"url\\(['\"]?([^'\"]*)['\"]?\\)", 'gi'), "$1");
	else
